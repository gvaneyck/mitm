...
public class AMF3Decoder {
     ...
     */
    public Object decode(byte[] data, int length) throws EncodingException, NotImplementedException {
        dataBuffer = data;
        dataPos = 0;
        dataLength = length;
     ...
     */
    private byte readByte() throws EncodingException {
        if (dataLength <= dataPos) {
            throw new EncodingException("Read too far");
        }
     ...
     */
    private int readByteAsInt() throws EncodingException {
        int ret = readByte();
        if (ret < 0) {
            ret += 256;
        }
     ...
     */
    private byte[] readBytes(int length) throws EncodingException {
     ...
     * @author FluorineFX
     */
    private int readInt() throws EncodingException {
     ...
     */
    private double readDouble() throws EncodingException {
        long value = 0;
        for (int i = 0; i < 8; i++) {
     ...
     */
    private Date readDate() throws EncodingException {
        int handle = readInt();
        boolean inline = ((handle & 1) != 0);
        handle = handle >> 1;
        ...
        if (inline) {
            ...
            if (cd.externalizable) {
                if (cd.type.equals("DSK")) {
                    ...
                    for (int i = dataPos; i < dataLength; i++) {
     ...
     */
    private byte[] readByteArray() throws EncodingException {
        int handle = readInt();
        boolean inline = ((handle & 1) != 0);
        handle = handle >> 1;
    ...
    private List<Integer> readFlags() throws EncodingException {
